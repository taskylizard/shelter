...
impl<'a> ThisToSelfTransform<'a> {
    ...
    fn belonging_function(ctx: &mut oxc_traverse::TraverseCtx<'_>, include_arrows: bool) -> Option<ScopeId> {
    ...
    fn self_name(&mut self, ctx: &mut oxc_traverse::TraverseCtx<'a>) -> Atom<'a> {
        match &self.current_self_name {
            ...
            None => {
                let name = ctx.generate_uid_in_current_scope("self$", SymbolFlags::ConstVariable).name;
    ...
    fn make_self_binding_stmt(name: Atom<'a>, ctx: &mut oxc_traverse::TraverseCtx<'a>) -> ast::Statement<'a> {
        ctx.ast.statement_declaration(ctx.ast.declaration_variable(
            ...
            ctx.ast.vec1(ctx.ast.variable_declarator(
                SPAN,
                ast::VariableDeclarationKind::Const,
                ctx.ast.binding_pattern(ctx.ast.binding_pattern_kind_binding_identifier(SPAN, name), NONE, false),
...
impl<'a> Traverse<'a> for ThisToSelfTransform<'a> {
        ...
        match node {
            ThisExpression(this) => {
                ...
                if current == self.current_parent {
                    ...
                    *node = ctx.ast.expression_identifier_reference(this.span, self_name);
                }
            }
            JSXElement(_) => {
                if self.jsx_depth == 0 {
    ...
    fn exit_expression(&mut self, node: &mut ast::Expression<'a>, ctx: &mut oxc_traverse::TraverseCtx<'a>) {
        use ast::Expression::*;
                        ...
                        None => {
                            let arrow_fn = ctx.ast.arrow_function_expression(
                                ...
                                ctx.ast.function_body(
                                    SPAN,
                                    ctx.ast.vec(),
                                    ctx.ast.vec1(ThisToSelfTransform::make_self_binding_stmt(self_name.clone(), ctx)),
                                ),
                            );
                            arrow_fn.scope_id.set(Some(ctx.create_child_scope_of_current(ScopeFlags::Arrow)));
                            *node = ctx.ast.expression_call(
                                SPAN,
                                ctx.ast.expression_parenthesized(SPAN, ctx.ast.expression_from_arrow_function(arrow_fn)),
                                NONE,
                                ctx.ast.vec(),
                                false,
                            );
    ...
    fn enter_jsx_member_expression(&mut self, node: &mut ast::JSXMemberExpression<'a>, ctx: &mut oxc_traverse::TraverseCtx<'a>) {
        if self.jsx_depth == 0 {
            return;
        }
        ...
        if let ast::JSXMemberExpressionObject::ThisExpression(_) = node.object {
            ...
            node.object = ctx.ast.jsx_member_expression_object_identifier_reference(SPAN, self_ref);
        }
    }

    fn exit_function_body(&mut self, node: &mut ast::FunctionBody<'a>, ctx: &mut oxc_traverse::TraverseCtx<'a>) {
            ...
            if b.scope_id == scope_id {
                ...
                let index = match is_constructor {
                    true => node
                        ...
                        .iter()
                        .position(|s| match s {
                            ast::Statement::ExpressionStatement(stmt) => stmt.expression.is_super_call_expression(),
                            _ => false,
                        })
                        .map(|i| i + 1)
                        .unwrap_or(0),
    ...
    fn exit_arrow_function_expression(&mut self, node: &mut ast::ArrowFunctionExpression<'a>, ctx: &mut oxc_traverse::TraverseCtx<'a>) {
        if node.expression && node.body.statements.len() > 1 {
            if let Some(ast::Statement::ExpressionStatement(_)) = node.body.statements.last() {
                ...
                node.body
                    .statements
                    .push(ctx.ast.statement_return(SPAN, Some(ctx.ast.move_expression(&mut expr.expression))));
