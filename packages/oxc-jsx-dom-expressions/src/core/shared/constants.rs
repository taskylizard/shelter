...
pub static PROP_ALIASES_OBJ: Lazy<HashMap<&str, HashMap<&str, &str>>> = Lazy::new(|| {
    HashMap::from([
        ("formnovalidate", HashMap::from([("$", "formNoValidate"), ("BUTTON", "1"), ("INPUT", "1")])),
        ("ismap", HashMap::from([("$", "isMap"), ("IMG", "1")])),
        ("nomodule", HashMap::from([("$", "noModule"), ("SCRIPT", "1")])),
        ("playsinline", HashMap::from([("$", "playsInline"), ("VIDEO", "1")])),
        ("readonly", HashMap::from([("$", "readOnly"), ("INPUT", "1"), ("TEXTAREA", "1")])),
    ])
});
...
pub static ALIASES: Lazy<HashMap<&str, &str>> = Lazy::new(|| HashMap::from([("className", "class"), ("htmlFor", "for")]));

pub static CHILD_PROPERTIES: Lazy<HashSet<&str>> = Lazy::new(|| HashSet::from(["innerHTML", "textContent", "innerText", "children"]));
